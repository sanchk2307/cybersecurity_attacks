...
def coln_max( row , timedim ) :
    ...
    # return "{ " + " + ".join( cols_target.str.replace( f"n { timedim } " , "" , regex = False )) + " }"
...
for target_col in target_cols :  
    ...
    for timedim in [ "d" ] :
        ...
        df_n[ df_n_name ] = df_n[ df_n_name ].groupby([ "date dd" , target_col ]).size().unstack().fillna( 0 )
        
        target_col_values = df[ target_col ].value_counts().index
        
        max_n_d_col = f"{ target_col } max n { timedim }"
        df_n[ df_n_name ][ max_n_d_col ] = df_n[ df_n_name ][[ value for value in target_col_values ]].apply( lambda x : coln_max( x , timedim ) , axis = 1 )
                
        for lags in [ 0 , 1 ] :
            if ( lags == 0 ) :
                max_n_dlags_col = max_n_d_col
                df[ max_n_dlags_col ] = df[ "date dd" ].map( df_n[ df_n_name ][ max_n_d_col ])
                for tarcol in df[ target_col ].value_counts().index :
                    df[ f"{ tarcol } n d exp" ] = df[ "date dd" ].map( df_n[ df_n_name ][ tarcol ].apply( math.exp ))
                    df[ f"{ tarcol } n d ln" ] = df[ "date dd" ].map( df_n[ df_n_name ][ tarcol ].apply( ln ))
            else :
                max_n_dlags_col = f"{ max_n_d_col }-{ lags }"
                df[ max_n_dlags_col ] = ( df[ "date dd" ] - pd.Timedelta( days = lags )).map( df_n[ df_n_name ][ max_n_d_col ])
...
crosstabs_x_AttackType = sankey_diag([
...
# -----------------------------------------------------------------------------
# Tests whether there is a significant association between a variable and Attack Type
...
for target in [ "Anomaly Scores" , "Source Port"  , "Destination Port" ] :
    ...
    for row , attype in enumerate( attypes ) :  
        ...
        for lvl in df[ f"Anomaly Scores classes , bias { attype }" ].unique()[ 1 : ] :
            vals = df[ df[ f"Anomaly Scores classes , bias { attype }" ] == lvl ][ "Anomaly Scores" ]
            fig.add_trace(
                go.Histogram(
                    x = vals ,
...
#%% Time Series Analysis : number Attack Type over time
# -----------------------------------------------------------------------------
...
#%% Time Series Analysis : number Attack Type x variable over time
# ----------------------------------------------------------------------------

target_col = "Network Segment"
df_TSA_pday = {}
df[ "date" ] = pd.to_datetime( df[ "date" ])
df_TSA_pday[ target_col ] = df.copy( deep = True )
df_TSA_pday[ target_col ][ "date_dd" ] = df_TSA_pday[ target_col ][ "date" ].dt.floor( "d" )
df_TSA_pday[ target_col ] = df_TSA_pday[ target_col ].groupby([ "date_dd" , target_col , "Attack Type" ]).size().unstack().fillna( 0 )
...
fig = subp(
    ...
    subplot_titles = [ f"Attack Type = { attype } , { target_col } = { target_value } per Day"
...
fig = subp(
    rows = len( attypes ) * 2 ,
    cols = len( target_values ) ,
    subplot_titles = [ f"{ metric } , Attack Type = { attype } , { target_col } = { target_value } per Day"
                      for target_value in target_values
...
for ( a , attype ) , ( v , value ) in product( enumerate( attypes ) , enumerate( target_values )) :
    # compute ACF
